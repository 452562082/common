...
func (zom *zookeeperOffsetManager) commitOffset(topic string, partition int32, tracker *partitionOffsetTracker) error {
	...
	if err != nil {
		log.Errorf("FAILED to commit offset %d for %s/%d!", tracker.highestProcessedOffset, topic, partition)
	} else if zom.config.VerboseLogging {
		log.Infof("Committed offset %d for %s/%d!", tracker.lastCommittedOffset, topic, partition)
	}
