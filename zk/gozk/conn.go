...
func (c *Conn) loop() {
	for {
		...
		case err != nil && c.conn != nil:
			...
			c.conn.Close()
		case err == nil:
			log.Infof("Authenticated: id=%d, timeout=%d", c.SessionID(), c.sessionTimeoutMs)
			...
			go func() {
				...
				log.Warnf("Send loop terminated: err=%v", err)
			...
			go func() {
				...
				log.Warnf("Recv loop terminated: err=%v", err)
				if err == nil {
...
func (c *Conn) sendSetWatches() {
	...
	go func() {
		...
		if err != nil {
			log.Errorf("Failed to set previous watches: %s", err.Error())
		}
	}()
}
...
func (c *Conn) recvLoop(conn net.Conn) error {
	...
	for {
		...
		if res.Xid == -1 {
			...
			log.Warnf("Xid < 0 (%d) but not ping or watcher event", res.Xid)
		} else {
			if res.Zxid > 0 {
				c.lastZxid = res.Zxid
			}
			...
			if !ok {
				log.Infof("Response for unknown request with xid %d", res.Xid)
			} else {
				if res.Err != 0 {
					err = res.Err.toError()
				} else {
